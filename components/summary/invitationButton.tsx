...
function encodeGroupId(groupId: number) {
  return Buffer.from(groupId.toString()).toString("base64")
}
...
export default function InvitationButton({ group_id }: { group_id: number }) {
  ...
  const handleGenerateInvitation = () => {
    ...
    const url = `${window.location.origin}/link?group_id=${encodedGroupId}`
  ...
  return (
    <Card className="w-full max-w-md my-5">
      <CardHeader>
        ...
        <CardDescription>Generate and share an invitation link for your group</CardDescription>
      </CardHeader>
      <CardContent>
        {!invitationUrl ? (
