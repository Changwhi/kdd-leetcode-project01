...
export const SelectEvent = ({ allEvents }: { allEvents: EventType[] }) => {
  return (
    <div className="bg-slate-50 md:p-2 md:mt-4 lg:p-4 rounded-xl lg:h-[78vh] md:w-full md:h-[40vh]">
      ...
      <Row className="md:h-[80%] lg:h-[65vh] overflow-y-auto">
        {allEvents.map((event) => (
          <Card className="items-center p-4 m-3 shadow-md hover:bg-gray-100 hover:shadow-lg transition-all duration-300 cursor-pointer" key={event.event_id} onClick={()=>{console.log(event.event_id
          )}}>
